...
export default function CertificadoPage() {
  ...
  // Update studentName when user data is available
  useEffect(() => {
    if (user?.name) {
  ...
  const handleGenerateCertificate = async () => {
    ...
    try {
      try {
        // Usa o Proxy interno para evitar CORS e erros de rede
        const response = await fetch(`/api/certificate`, {
          ...
          body: JSON.stringify({ safeStudentName: safeName }),
        });
        ...
        <section className="flex flex-col items-center w-full p-4 md:p-8">
          ...
          <div className="w-full max-w-lg bg-gray-800 p-6 md:p-8 rounded-lg shadow-lg text-center">
            ...
            <button
              ...
              className={`w-full px-6 py-3 font-bold text-white rounded-md transition-colors flex items-center justify-center ${isGenerating ? 'bg-gray-500 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'
                }`}
            >
              {isGenerating ? (
                <>
